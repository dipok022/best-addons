 ...
 */
class Best_Addons_Widget_Builder_Admin_UI
{
	...
	{
		// Main list page under Best Addons settings.
		add_submenu_page(
	...
	{
		require_once __DIR__ . '/class-list-page.php';
		Best_Addons_Widget_Builder_List_Page::render();
	}
	...
	public static function maybe_render_editor()
	{
		if (
			! is_admin() ||
			! isset($_GET['page']) ||
			'ba-widget-builder-edit' !== sanitize_key(wp_unslash($_GET['page']))
		) {
			return;
		}
		...
		require_once __DIR__ . '/class-editor-page.php';
		Best_Addons_Widget_Builder_Editor_Page::render(); // outputs full HTML + exit
	}
	...
	public static function enqueue_list_assets($hook)
	{
		...
		if ('best-addons_page_ba-widget-builder' !== $hook) return;

		// jQuery UI for sortable.
		wp_enqueue_script('jquery-ui-sortable');

		// Our list page JS.
		$dist_js = plugin_dir_path(dirname(dirname(__FILE__))) . 'assets/dist/js/widget-builder-list.min.js';
		$src_js  = plugin_dir_path(dirname(dirname(__FILE__))) . 'assets/src/js/widget-builder-list.js';

		if (file_exists($dist_js)) {
			$js_url = plugins_url('assets/dist/js/widget-builder-list.min.js', dirname(dirname(__FILE__)));
			$ver    = filemtime($dist_js);
		} elseif (file_exists($src_js)) {
			$js_url = plugins_url('assets/src/js/widget-builder-list.js', dirname(dirname(__FILE__)));
		...
		if ($js_url) {
			wp_enqueue_script('ba-widget-builder-list', $js_url, ['jquery', 'jquery-ui-sortable'], $ver, true);
			wp_localize_script('ba-widget-builder-list', 'baWidgetBuilder', [
		...
		// Our list page CSS.
		$dist_css = plugin_dir_path(dirname(dirname(__FILE__))) . 'assets/dist/css/widget-builder-admin.min.css';
		if (file_exists($dist_css)) {
			$css_url = plugins_url('assets/dist/css/widget-builder-admin.min.css', dirname(dirname(__FILE__)));
			$css_ver = filemtime($dist_css);
			wp_enqueue_style('ba-widget-builder-admin', $css_url, [], $css_ver);
		}
	}
	...
	public static function ajax_create_widget()
	{
		...
		$category = isset($_POST['category']) ? sanitize_key(wp_unslash($_POST['category'])) : 'best-addons-category';
